...
func doJSON(t *testing.T, h http.Handler, method, path, ip string, body any) *httptest.ResponseRecorder {
    ...
    if ip != "" { req.Header.Set("X-Forwarded-For", ip) }
...
func TestDownloadSingle_Success(t *testing.T) {
    ...
    if ct := w.Header().Get("Content-Type"); ct != "application/json" { t.Fatalf("content-type=%s", ct) }
    var resp struct{ Status, Message, ID string }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatal(err) }
    if resp.Status != "success" || resp.ID != "abc123" { t.Fatalf("resp=%+v", resp) }
}
...
func TestDownloadSingle_MethodNotAllowed(t *testing.T) {
    ...
    if resp["message"] != "method_not_allowed" { t.Fatalf("resp=%v", resp) }
}
...
func TestDownloadSingle_InvalidURL(t *testing.T) {
    ...
    var resp map[string]any
    _ = json.Unmarshal(w.Body.Bytes(), &resp)
    if resp["message"] != "invalid_url" { t.Fatalf("resp=%v", resp) }
}
...
func TestDownloadSingle_ErrorMappings(t *testing.T) {
    ...
    for _, tc := range cases {
        t.Run(tc.name, func(t *testing.T) {
            ...
            var resp map[string]any
            _ = json.Unmarshal(w.Body.Bytes(), &resp)
            if resp["message"] != tc.msg { t.Fatalf("msg=%v", resp["message"]) }
        })
    }
}
...
func TestBatch_Success_Mixed(t *testing.T) {
    ...
    h := New(&mockMgr{
        enqueueFn: func(url string) (string, error) {
            calls++
            if strings.Contains(url, "a") { return "id-a", nil }
            if strings.Contains(url, "b") { return "", errString("queue_full") }
            return "", nil
        },
        snapshotFn: func(id string) []*download.Item { return nil },
    })
    ...
    if w.Code != http.StatusOK { t.Fatalf("status=%d body=%s", w.Code, w.Body.String()) }
    var resp struct{ Status, Message string; IDs []string `json:"ids"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatal(err) }
    if resp.Status != "success" || len(resp.IDs) != 1 || resp.IDs[0] != "id-a" { t.Fatalf("resp=%+v", resp) }
    if calls != 2 { t.Fatalf("expected 2 enqueue calls (valid URLs), got %d", calls) }
}
...
func TestStatus_AllAndByID(t *testing.T) {
    ...
    h := New(&mockMgr{
        ...
        snapshotFn: func(id string) []*download.Item { if id == "" { return items }; return []*download.Item{{ID: id, URL: "uX", Progress: 0, State: download.StateQueued}} },
    })
    // all
    w := doJSON(t, h, http.MethodGet, "/api/status", "10.0.0.8", nil)
    if w.Code != http.StatusOK { t.Fatalf("status=%d", w.Code) }
    var resp struct{ Status string; Downloads []download.Item }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatal(err) }
    if len(resp.Downloads) != 2 { t.Fatalf("downloads=%d", len(resp.Downloads)) }
    // by id
    ...
    if err := json.Unmarshal(w2.Body.Bytes(), &resp2); err != nil { t.Fatal(err) }
    if len(resp2.Downloads) != 1 || resp2.Downloads[0].ID != "abc" { t.Fatalf("resp=%+v", resp2) }
}
...
func TestRateLimiting_Exceeds(t *testing.T) {
    ...
    if last.Code != http.StatusTooManyRequests { t.Fatalf("expected 429, got %d", last.Code) }
    var resp map[string]any
    _ = json.Unmarshal(last.Body.Bytes(), &resp)
    if resp["message"] != "rate_limited" { t.Fatalf("resp=%v", resp) }
}
